...
from ai_art.locks import pipeline_lock
...
def _run_pipeline(args: argparse.Namespace, on: date) -> int:
    ...
    if not args.skip_publish:
        try:
            out = publish_site(message=f"AI art {on.isoformat()} — {artist.name}")
            log(f"publish: {out}")
